...
if ($port_htts -ne "") {
  Write-Host "  Порт https:" $port_https
}
Write-Host ""
...
if ($port_https -ne "")
{
  $out = (netstat -an | findstr /i :$port_https)  | Out-String

  if ($out -ne "") {
    Write-Host "Порт $port_https занят" -ForegroundColor Red
    Write-Host $out
    $is_error = $true
  } else {
    Write-Host "Порт свободен: " $port_https -ForegroundColor Green
  }
}
...
foreach ($line in $cfg){
  ...
  } elseif (($port_https -ne "") -and ($line.StartsWith("    https_port:"))){
    Add-Content -Path $dst_path\etc\config.yml -Value "    https_port: $port_https" -Encoding 'utf8' 
  } else {
...
elseif ($version -in @("4.10", "4.11", "4.12", "4.13"))
{
...
elseif ($version -in @("25.1", "25.2", "25.3", "26.1"))
{
  Write-Host @("25.1", "25.2", "25.3", "26.1")
  .\do.bat components add_package $rx_instaler_dir_path\Platform.zip
  .\do.bat components add_package $rx_instaler_dir_path\DevelopmentStudio.zip 
