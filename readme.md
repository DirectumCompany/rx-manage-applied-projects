...
* `do map dds_wo_deploy` - открыть DevelopmentStudio без фактического переключения на проект
...
1. Установить RX штатными средствами DirectumLauncher
2. Доработать стандартный config.yml
3. Создать файл описания текущего проекта
...
После подготовки файла описания проекта переключитесь на него командой `do map set` и проверьте его работоспособность:

* откройте и залогиньтесь в веб-доступе
* откройте тело какого-либо  документа (например шаблона)
* запустите DDS (командой `do dds run`) и опубликуйте разработку.
...
* для использования разных версий RX;
* для использования разных серверов БД в разных инстансах.
...
Однако остается еще несколько мест, в которых инстансы могут конфликтовать друг с другом:

* лог-файлы.
* очереди в RabittMQ

### Подготовка к RX к работе с несколькими инстансами

Предполагается, что RX уже был подготовлен для переключения между разными проектами внутри одного инстанса.

#### Скорректируйте config.yml

1. В раздел variables добавьте переменную:
   * instance_name - имя инстанса. Рекомендуется в имени инстанса отражать его суть. Например, `rx4227` - инстанс для RX 4.2.27. 
...
3. В разделе common_config:
   - в параметре QUEUE_CONNECTION_STRING для в значение параметра Exchange добавить переменную {{ instance_name }}. Например, `Exchange=rxexch_{{ instance_name }}`. Необходимо для того, чтобы разные инстансы не "воровали" друг у друга сообщения.
4. В разделе services_config->DevelopmentStudio:
   * параметра SERVICE_RUNNER_CONFIG_PATH задать значением вида `C:\rx\4227\etc\_{{ instance_name }}\_services_config\ServiceRunner\_ConfigSettings.xml`, где `C:\rx\4227\` - корневой каталог инстанса.
...
services_config:
    ...
    DevelopmentStudio:
        ...
        SERVICE_RUNNER_CONFIG_PATH: 'C:\rx\4227\etc\_{{ instance_name }}\_services_config\ServiceRunner\_ConfigSettings.xml'
```

где `C:\rx\4227\` - корневой каталог инстанса.

#### Перенастройте установленный RX для работы в варианте с несколькими инстансами

Порядок:

* остановить все сервисы RX `do all down`
* удалите компоненты Manage Applied Tools командой `do components delete map` или `do uninstall_plugin map` в зависимости от способа установки компоненты
* создайте папку для инстанса RX. Например, `c:\rx\4227` или `c:\rx4227'.
* скопируйте в созданную папку содержимое папки установленного ранее RX
* в папке инстанса удалите папки  `etc\_builds_bin`, `etc\_builds_cache`, `etc\_builds_package` и `etc\_services_config` 
* заново установите компоненту Manage Applied Tools
* в новом инстансе выполните `do iis configure` - она создаст и настроит пул приложений и веб-сервер в IIS. У созданных пула приложений и веб-сервера в имени будет использоваться имя инстанса в качестве суффикса.
* переключитесь на ранее созданный проект командой `do map set` и проверьте работоспособность:
  * откройте и залогиньтесь в веб-доступе
  * откройте тело какого-либо  документа (например шаблона)
  * запустите DDS (командой `do dds run`) и опубликуйте разработку
* если всё корректно - можно удалить пул приложений и веб-сервер, созданные в ходе установки RX штатными средствами

### Установка дополнительного инстанса RX

* создайте папку для нового инстанса. Например, `c:\rx\4427` или `c:\rx4427'
* распакуйте в папку инстанса содержимое архива DirectumLauncher.zip требуемой версии RX
...
* подберите свободный порт(ы) для будущего инстанса. Проверить свободен тот или иной порт можно командой `netstat -an | findstr /i :номер_порта`. Если порт свободен, то команда ничего не вернет
* сконфигурируйте config.yml для инстанса. Есть два способа:
  * первый - взять заготовку config.yml для соответствующей версии RX из дистрибутива компоненты Manage Applied Tools и заполнить необходимые параметры (см. параметры, в значениях которых есть символы `...`)
  * второй - скопировать config.yml из уже развернутого инстанса rx и доработать его:
    * изменить значения переменных instance_name, http_port и https_port
    * изменить значение параметра SERVICE_RUNNER_CONFIG_PATH в разделе DevelopmentStudio
* в новом инстансе выполните `do iis configure` - она создаст и настроит пул приложений и веб-сервер в IIS. У созданных пула приложений и веб-сервера в имени будет использоваться имя инстанса в качестве суффикса
* установите компоненту Manage Applied Tools
* создайте файл описания первого проекта для нового инстанса. Рекомендуется создать проект, в котором  будет только стандартная прикладная разработка
* создайте первый проект для инстанса командой `do map create_project` и проверьте работоспособность:
  * откройте и залогиньтесь в веб-доступе
  * откройте тело какого-либо  документа (например шаблона)
  * запустите DDS (командой `do dds run`) и опубликуйте разработку.
